...
LUA_SHA1 = get_hash(os.path.join(CWD, 'naver-matome.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--warc-header', 'naver-matome-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('naver-matome-item: %(item_name)s'),
        ...
        if item_type == 'profile':
            wget_args.extend(['--warc-header', 'naver-matome-profile: ' + item_value])
            wget_args.append('https://matome.naver.jp/mymatome/' + item_value)
        elif item_type == 'topic':
            wget_args.extend(['--warc-header', 'naver-matome-topic: ' + item_value])
            wget_args.append('https://matome.naver.jp/topic/' + item_value)
        else:
...
project = Project(
    title = 'NAVERまとめ',
    project_html = '''
    <img class="project-logo" alt="logo" src="https://archiveteam.org/images/7/77/Naver-matome-logo.jpg" height="50px"/>
    <h2>matome.naver.jp <span class="links"><a href="https://matome.naver.jp/">Website</a> &middot; <a href="http://tracker.archiveteam.org/naver-matome/">Leaderboard</a></span></h2>
    '''
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='naver-matome'),
    WgetDownload(
        WgetArgs(),
        max_tries=1,
